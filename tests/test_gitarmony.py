...
class GitarmonyTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.gitarmony = Gitarmony.install(
            self.gitarmony_origin_url, self.managed_clone.working_dir, hooks=False, platform='GitHub'
        )
    ...
    def test_local_commits(self):
        ...
        image_path = os.path.join(working_dir, "staged_image_01.jpg")
        save_image(image_path)
        self.managed_clone.index.add(image_path)
        self.assertEqual(5, len(self.gitarmony.local_commits[0]["changes"]))

        commit = self.managed_clone.index.commit(message="Add staged_image.jpg")
        local_commits = self.gitarmony.local_commits
        self.assertEqual(2, len(local_commits))
        self.assertEqual(4, len(local_commits[0]["changes"]))
        ...
        conflicting_commit = self.gitarmony.get_conflicting_commit("staged_image_02.jpg")
