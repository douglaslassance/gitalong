...
class GitalongTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.gitalong_remote_url = os.path.join(self.temp_dir, "gitalong.git")
        self.gitalong_remote = Repo.init(path=self.gitalong_remote_url, bare=True)
        self.gitalong = Gitalong.install(
            self.gitalong_remote_url,
    ...
    def test_config(self):
        config = self.gitalong.config
        self.assertEqual(
            os.path.normpath(self.gitalong_remote_url),
    ...
    def test_local_only_commits(self):
        ...
        self.assertEqual(4, len(self.gitalong.local_only_commits[0]["changes"]))
        ...
        # pprint(self.gitalong.get_tracked_commits())
        ...
        last_commit = self.gitalong.get_file_last_commit("staged_image_02.jpg")
        spread = self.gitalong.get_commit_spread(last_commit)
        self.assertEqual(
        ...
        missing_commit = self.gitalong.make_file_writable(staged_image_01_path)
        self.assertEqual(False, bool(missing_commit))
        missing_commit = self.gitalong.make_file_writable(image_path)
